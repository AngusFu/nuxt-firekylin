$(document).ready(function() {
	...
	$.getJSON('https://raw.githubusercontent.com/Fyrd/caniuse/master/fulldata-json/data-2.0.json', function(res) {
		...
		for (var i = 0; i < featuresArray.length; i++) {
			...
			$('select[name="featureID"]').append(option).select2();
		}
	...
	$('input[type="submit"]').on('click', function() {
		...
		var stepFour = document.getElementById('copyStepFour');
		var stepFourClipboard = new Clipboard(stepFour);
		...
		var i=window.addEventListener?"addEventListener":"attachEvent",d=window[i],l="attachEvent"==i?"onmessage":"message";d(l,function(t){
			$('#stepFour').text( $('.export-preview p').html() );
		});
